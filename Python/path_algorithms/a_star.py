...
class Grid:
    def __init__(self, x, y):
        np.random.seed()
        self.nodes = np.random.randint(20, size=(x, y))
        self.walls = []
        ## create some random walls
        for _ in range(6):
            begin = np.random.randint(10)
            offset = np.random.randint(30)
            direction = np.random.randint(2)
            for i in range(np.random.randint(31-begin)):
                if direction:
                    self.walls.append((begin+i, offset))
                else:
                    self.walls.append((offset, begin+i))
...
class Pathfinder:
    ...
    def expand(self):
        while True: # pop queue until unexplored node
            if self.frontier.empty(): self.get_best(); return # there is no path
            current = self.frontier.get()[1]
            if current not in self.explored: break
        ...
        for next_node in G['grid'].neighbors(current):
            if next_node in self.explored: continue # skip all explored nodes
            ...
            if next_node not in self.cost_so_far or new_cost < self.cost_so_far[next_node]:
                ...
                d = manhattan_dist(next_node, self.goal)
                priority = new_cost + d * GOAL_COEFF
    ...
    def get_best(self):
        best = None
        for node in self.cost_so_far:
            if not best: best = node; continue
            if manhattan_dist(node, self.goal) < manhattan_dist(best, self.goal): best = node
            elif manhattan_dist(node, self.goal) == manhattan_dist(best, self.goal):
                if self.cost_so_far[node] < self.cost_so_far[best]: best = node
        path = []
        while best != self.start:
            path.append(best)
            best = self.came_from[best]
        path.reverse()
        ## coloring pass
        for x in range(G['grid'].nodes.shape[0]):
            for y in range(G['grid'].nodes.shape[1]):
