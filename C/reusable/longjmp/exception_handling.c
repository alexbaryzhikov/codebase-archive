...
like try in some other languages. The call to longjmp is analogous
to a throw statement, allowing an exception to return an error
status directly to the setjmp.
*/
#include <setjmp.h>
#include <stdio.h>
#include <stdlib.h>
...
int main( void ) {
	...
	free( mem_buffer );  /* NULL can be pased to free, no operation is performed */
